...
public interface PostRepository extends JpaRepository<Post, Long> {
    @Query("select new com.ssafy.backend.domain.post.dto.PostInfoResponse(p.id, p.title, p.state, u.profileImage, u.profileColor, (select count(*) from Reply where Reply.post = p)) from Post p left join User u on p.user = u where p.chatRoom.id = :chatRoomId")
    List<PostInfoResponse> findByChatRoomId(@Param("chatRoomId") Long chatRoomId);
