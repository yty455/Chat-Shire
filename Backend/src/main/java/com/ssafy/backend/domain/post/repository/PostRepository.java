...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query("select new com.ssafy.backend.domain.post.dto.PostInfoDetailResponse(p.id, p.title, p.content, (select s.skillName from PostSkill ps left join ps.skill s where ps.post.id = :postId), u.id, u.githubId,u.profileImage, u.profileColor, u.nickname, p.state, p.createdDate, p.lastModifiedDate) from Post p left join User u on p.user = u where p.id = :postId")
