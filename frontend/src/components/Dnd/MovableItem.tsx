...
const MovableItem = ({
  ...
  const [{ isDragging }, drag] = useDrag({
    ...
    item: { id, githubId },
      ...
      if (dropResult) {
        ...
        const { indiTask, teamTask } = COLUMN_NAMES;
        switch (name) {
          case teamTask:
            changeItemColumn(item, teamTask);
            break;
          case indiTask:
            changeItemColumn(item, indiTask);
            break;
        }
      }
    },
  ...
  return (
    <div ref={ref} className={styles.MemberItemContainer} style={{ opacity }}>
      <div
        style={{ backgroundColor: profileColor ? profileColor : "red" }}
      ...
      <span style={{ color: "#575757" }}>{githubId}</span>
    </div>
  );
};
