...
export default function RightFileTab() {
  ...
  const listFiles = (): Promise<{url: string, name: string, size: number}[]> => {
        ...
        if (err) {
          ...
          if (data.Contents) {
            let files = data.Contents.map((file:any) => {
              let url = s3.getSignedUrl('getObject', {Bucket:"chat-shire", Key: file.Key});
  ...
  return (
    <div className={styles.MessageRightBody}>
      ...
      <div className={styles.MessageFileStorage}>
        {files.length !== 0 ? files.map((fileInfo, index) => (
          <div className={styles.fileThumbnail} key={index}>
            <CardActionArea 
              onClick={() => window.open(fileInfo.url, "_blank")} 
              style={{height: "100px", display: "flex", flexDirection: "column", justifyContent: "space-between", alignItems: "flex-start", padding: "10px"}}
            >           
              <span style={{fontFamily:'preBd'}}>{fileInfo.name}</span>
              <span style={{fontFamily:'preLt'}}>{(fileInfo.size / 1024).toFixed(2)} KB</span>
            </CardActionArea>
          </div>
        )) : (
