...
export default function SetMember({
  ...
  onData: (membersData: string[]) => void;
}) {
  ...
  const handleSearchInput = (e: any) => {
    ...
    api
      .get("users/search?githubId=" + e.target.value)
      .then((res) => {
  ...
  const handleInviteBtnChange = (e: any) => {
    if (e.target.checked) {
      ...
      const newMembers = members.filter(
        (item) => item.id !== String(e.target.id)
      );
      const newInvitedMembers = invitedMembers.filter(
        (item) => item !== String(e.target.id)
      );
    ...
    return (
      <div className={styles.SearchResultItem}>
        <div style={{ display: "flex", alignItems: "center" }}>
          ...
          >
            <span style={{ fontFamily: "preRg", fontSize: "14px" }}>
              {item.githubId}
            </span>
            <span style={{ fontFamily: "preRg", fontSize: "14px" }}>
              {item.nickname}
        ...
        {invitedMembers.includes(String(item.id)) ? (
          <Checkbox
            ...
            name={item.profileColor}
            onClick={handleInviteBtnChange}
            style={{ marginLeft: "280px" }}
            icon={<BsCircle size={26} />}
            checkedIcon={<BsCheckCircleFill size={26} />}
          />
        ) : (
          <Checkbox
            checked={false}
