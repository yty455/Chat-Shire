    ...
    <Box sx={{p: 0, pt: 1}}>
      ...
      {checkboxItems && checkboxItems.length !== 0 ? (
        ...
        <Grid sx={{margin: 0, padding:0 }} item xs={12} key={item.id}>
          <Item sx={{borderRadius: "0px 20px 20px 20px", margin:'0 10px', padding:0, minHeight: '30px'}} className={styles.oneMemo} elevation={7}>
            <div className={styles.indivTask}>
              ...
              {
                item.isEditing ?
                <input onKeyPress={handleKeyPress(item.id)} style={{fontFamily:'preRg',height: '30px', marginTop: '9px', border:'none'}} type="text" onBlur={handleContentChange(item.id)} placeholder="내용을 입력하세요"/> :
            ...
            <BsFillChatDotsFill style={{fontSize: '17px', margin: '-5px 5px 10px 0'}}/>
            <BsPencilFill style={{fontSize: '17px', margin: '-5px 3px 10px 0'}}/>
            <MdDelete style={{fontSize: '20px', margin: '-7px 10px 10px 0'}} onClick={() => handleDelete(item.id)}/>
            </div>
        ...
        <p style={{color: 'grey', fontFamily:'preBd',margin: '30px auto 0 auto',paddingLeft:'10px'}}>아직 등록된 태스크가 없습니다.</p>
      )}
      </Grid>
    </Box>
